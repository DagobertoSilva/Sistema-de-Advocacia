...
export default function Conversas() {
  ...
  useEffect(() => {
      ...
      fetch("http://localhost:8080/api/conversas")
        ...
        .then((data) => {
          const formatadas = data.map((conv) => {
            ...
            if (
              nomeOriginal &&
              nomeOriginal !== "Novo Lead - Whats" &&
              !nomeOriginal.startsWith("Lead #")
            ) {
              ...
              if (textoMinusculo.includes("meu nome é")) {
                textoTratado = textoTratado
                  .substring(textoMinusculo.indexOf("meu nome é") + 10)
                  .trim();
              } else if (textoMinusculo.includes("meu nome e")) {
                textoTratado = textoTratado
                  .substring(textoMinusculo.indexOf("meu nome e") + 10)
                  .trim();
              } else if (textoMinusculo.includes("me chamo")) {
                textoTratado = textoTratado
                  .substring(textoMinusculo.indexOf("me chamo") + 8)
                  .trim();
              }

              if (textoTratado.includes(".")) {
                textoTratado = textoTratado.split(".")[0].trim();
              }
              ...
              const palavras = textoTratado.split(/\s+/);
            ...
            const assuntoReal =
              conv.cliente?.assuntoTipificado || "Triagem Chatbot";
            ...
            return {
              ...
              urgencia:
                ...
                assuntoReal === "Roubo" ||
                assuntoReal === "Roubo de Veículo" ||
                assuntoReal === "Tráfico" ||
                assuntoReal === "preso em flagrante"
                  ? "Alta"
                  : "Normal",
              data: conv.dataInicio
                ? new Date(conv.dataInicio).toLocaleDateString("pt-BR")
                : "Recente",
            };
          });
    ...
    const intervalo = setInterval(carregarCasosEConversas, 5000);
  ...
  const handleSelecionarCliente = (cliente) => {
    ...
    fetch(
      `http://localhost:8080/api/chat/clientes/${cliente.id_cliente}/mensagens`,
    )
      .then((res) => res.json())
      .then((mensagens) => {
          ...
          {conversasFiltradas.map((cliente) => (
            ...
            >
              ...
              <div className="cliente-info-secundaria">
                ...
                <span
                  className={`badge-urgencia ${cliente.urgencia === "Alta" ? "alta" : "normal"}`}
                >
                  {cliente.urgencia}
                </span>
              </div>
      ...
      <div className="conteudo-conversas">
        {carregandoMensagens ? (
            ...
            <div className="conversa-header-detalhe">
              <div className="user-profile-summary">
                ...
                <div>
                  ...
                  <p>
                    WhatsApp: {clienteSelecionado.whatsapp} | Linha do Tempo da
                    Triagem
                  </p>
                </div>
              </div>
              ...
              {historicoMensagens.length > 0 ? (
                historicoMensagens.map((msg, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className={`chat-row ${IsCliente ? "row-cliente" : "row-bot"}`}
                    >
                ...
                <div className="sem-mensagens">
                  <AlertCircle size={32} />
                  <p>
                    Nenhuma mensagem de texto registrada para este lead nas
          ...
          <div className="status-container empty-state">
            ...
            <p>
              Selecione um cliente na barra lateral para auditar a transcrição
              completa das mensagens trocadas com o bot.
            </p>
          </div>
        )}
      </div>
